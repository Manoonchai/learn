...
<main class="main container min-h-screen  mx-auto flex dark:bg-black flex-col gap-2 justify-center items-center py-20">
  ...
  <p class="sentence">
    {#each sentence as word, idx (idx)}
      <span
        class="sentence-gap font-sarabun dark:text-white transition duration-200 break-word {idx === currentWordIdx
  ...
  />
  {#if $showPrevOrNextWord}
  <p class="flex-none word-next text-gray-400 dark:text-gray-200 w-32 text-xl pt-4 pl-6 font-sarabun">{sentence[currentWordIdx + 1] ?? ''}</p> 
  {/if}  
</div>
  ...
  <button class="btn hover:bg-gray-600 rounded-lg transition duration-300 m-2 dark:text-white" on:click={reset}>
    Reset
  </button>

  <div class="lesson dark:text-white font-sarabun">
    Lesson: <span class="p-2 cursor-pointer border border-gray-400 dark:text-white rounded-lg" on:click={() => (showLesson = true)}>{$currentLessonName}</span>
  {#if showLesson}
    <LessonModal closeModal={() => {(showLesson = false); reset()}} />
  {/if}
  </div>
